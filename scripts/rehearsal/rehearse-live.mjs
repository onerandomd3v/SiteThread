...
const child = spawn(testCommand, ["exec", "playwright", "test", "tests/rehearsal/live.spec.ts", "--config=playwright.rehearsal.config.ts"], { cwd: root, stdio: "inherit", env: { ...process.env, REHEARSAL_REFERENCE_FINGERPRINT: fingerprint, REHEARSAL_REFERENCE_DURATION: String(durationSeconds) } });
const exitCode = await new Promise((resolve) => child.on("exit", (code) => resolve(code ?? 1)));
if (exitCode !== 0) process.exit(exitCode);

const verify = spawn(testCommand, ["exec", "node", "scripts/rehearsal/verify-runs.mjs"], { cwd: root, stdio: "inherit", env: { ...process.env, REHEARSAL_REFERENCE_FINGERPRINT: fingerprint } });
