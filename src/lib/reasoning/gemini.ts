...
const REASONING_INSTRUCTION = [
  ...
  "Copy evidence refs exactly from the supplied labels. Each observation must contain exactly one atomic factual claim; split compound claims into separate observations.",
  ...
  "If no atomic claim is directly supported by its cited evidence, omit it; if no supported claims remain, return {\"observations\":[]}.",
].join(" ");
